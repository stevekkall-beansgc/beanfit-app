...
const MODEL = { mem_q4: 17.0, mem_q8: 30.0, kv32k: 1.8 };

test("fit math matches CLI engine reference numbers", () => {
  // CLI: total = 17.0 + 0.9 = 17.9; tok/s = 600/17.9*0.85 = 28.5
  const r = fits(MODEL, { model_budget_gib: 96, mem_bandwidth_gbs: 600 });
  assert.equal(r.quant, "q4_K_M");
  assert.equal(r.total_gib, 17.9);
  assert.equal(r.est_tok_s, 28.5);
});

test("prefers q4, falls back to q8 when q4 misses budget", () => {
  const tight = { model_budget_gib: 34 }; // q4 total 17.9 fits
  const tighter = { model_budget_gib: 16 }; // q4=17.9 no; q8=30.9 no
